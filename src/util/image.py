...
def upload_crop(image_path, ext):
    ...
    store_dir = _get_image_dir(config.UPLOAD_DIR)
    base_name = get_uuid()
    ...
    # middle
    dest_width = config.MIDDLE_WIDTH
    ...
    # thumb
    dest_width, dest_height = config.THUMB_SIZE
...
def icon_crop(user_id, icon_path, coords):
    ...
    store_dir = _get_image_dir(config.ICONS_DIR)
    ...
    big_size = (config.ICON_BIG_WIDTH, config.ICON_BIG_WIDTH)
    img_thumb = img_thumb.resize(big_size, Image.ANTIALIAS)
    img_thumb.save(middle_path, quality=150)
    
    thumb_size = (config.ICON_WIDTH, config.ICON_WIDTH)
